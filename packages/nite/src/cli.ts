...
cli
  ...
  .action(async (root: string, options: RunnerOptions & GlobalCLIOptions) => {
    filterDuplicateOptions(options)
    const { createServer } = await import('./runner')
    try {
      await createServer({
        root,
        base: options.base,
        ...
        runner: cleanGlobalCLIOptions(options),
        forceOptimizeDeps: options.force,
      })
      // const info = server.config.logger.info

      const niteStartTime = global.__nite_start_time ?? false
      const startupDurationString = niteStartTime
        ? colors.dim(
          `ready in ${colors.reset(
            colors.bold(Math.ceil(performance.now() - niteStartTime)),
          )} ms`,
        )
        : ''
